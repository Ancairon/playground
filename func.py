        ...
        if file.endswith(".conf"):
            ...
            for line in whole_file:
                ...
                if not line.startswith("#"):
                    ...
                    # # print(i, len(whole_file))
                    if name and (line == "\n" or i==len(whole_file)-1):
                        try:

                            # nest_dict = {"name": name, "metric": metric, "os": operating_sys, "info": info}
                            nest_dict = {"name": f"[{name}](https://gihtub.com/netdata/netdata/blob/master/health/health.d/{filename})", "metric": metric, "info": info}
...
duplicate_for_virtual_integrations:
  list:
    - title: Ubuntu
      description: dummy description that will replace the original "overview" section, leave the whole list empty if none
    - title: Redhat
      description: dummy description that will replace the original "overview" section, leave the whole list empty if none
overview:
  operation:
    description: This collector periodically does http requests to one or more [Apache](link to collector) web servers to collect the metrics mod_status plugin that apache exposes
  platforms:
    description: If this collector can't run on some platforms specify them here, otherwise set this to true and this text will not be rendered, instead a templated message will appear
    boolean: true
  multi-instance:
    boolean: true
  permissions:
    ...
    with open(csv_file) as f:
        ...
        for row in csv_data:
            ...
            try:
                ...
                print(module, "Module has alerts")
            except Exception as e:
                print("Exception" , e)
        ...
        for module in modules:
            ...
            template['metrics'] = ({"description":"some description for this section, or leave empty","folding":{"title": "Metrics", "enabled":False},"scope":metric[module]})
            template["name"] = module
            metricslist.append(template)
            print(template.keys())
            # metricslist.append({
...
for directory in next(os.walk(f'{dir}'))[1]:
    ...
    # print(alert_dict)#, alert_dict["system.ram"])
    try:
        csv_to_yaml(f'{dir}/{directory}/metrics.csv', f'{dir}/{directory}/metadata.yaml', alert_dict)  
    except Exception as e:
        print("Exception", e)
    # exit()
