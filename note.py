...
def myfunc(markdownFiles):
    ...
    alerting_agent = pd.DataFrame()
    alerting_cloud = pd.DataFrame()
    ...
    for file in markdownFiles:
        if file not in readmes_first:
            readmes_first.append(file)

    markdownFiles = readmes_first
    ...
    for file in markdownFiles:
        ...
        if path not in ignore_dup:
            dummy_file = open(file, "r")
            whole_file = dummy_file.read()
            dummy_file.close()
            if "DO NOT EDIT THIS FILE DIRECTLY" in whole_file:
                if os.path.islink(file):
                    ignore_dup.append(file.replace("README.md", ""))

                meta = whole_file.split("endmeta-->")[0].replace("<!--startmeta", "---") + "---"
                # print(meta)
                ...
                #here we need a different check, as the path variable gets messed up
                    ...
                    alerting_cloud = pd.concat([alerting_cloud, metadf])
                else:
                    print("in")
                    alerting_agent = pd.concat([alerting_agent, metadf])
                
    # print("Collectors\n", collectors_entries, "Agent alerts\n", alerting_agent, "Cloud alerts\n",  alerting_cloud, "Exporting",  exporting_entries)


    replace_index = map_file.loc[map_file['custom_edit_url'] == "collectors_integrations"].index
    # print(replace_index[0])
    upper = map_file.iloc[:replace_index[0]]
    lower = map_file.iloc[replace_index[0]+1:]

    map_file = pd.concat([upper, collectors_entries, lower], ignore_index=True)

    replace_index = map_file.loc[map_file['custom_edit_url'] == "agent_notifications_integrations"].index
    # print(replace_index[0])
    upper = map_file.iloc[:replace_index[0]]
    lower = map_file.iloc[replace_index[0]+1:]

    map_file = pd.concat([upper, alerting_agent.sort_values(by=['sidebar_label']), lower], ignore_index=True)

    replace_index = map_file.loc[map_file['custom_edit_url'] == "cloud_notifications_integrations"].index
    # print(replace_index[0])
    upper = map_file.iloc[:replace_index[0]]
    lower = map_file.iloc[replace_index[0]+1:]

    map_file = pd.concat([upper, alerting_cloud.sort_values(by=['sidebar_label']), lower], ignore_index=True)

    replace_index = map_file.loc[map_file['custom_edit_url'] == "exporters_integrations"].index
    # print(replace_index[0])
    upper = map_file.iloc[:replace_index[0]]
    lower = map_file.iloc[replace_index[0]+1:]

    map_file = pd.concat([upper, exporting_entries.sort_values(by=['sidebar_label']), lower], ignore_index=True)
...
def insert_and_read_hidden_metadata_from_doc(path_to_file, dictionary):
    ...
    for field in dictionary.loc[dictionary['custom_edit_url'] == key]:
        try:
            ...
            # pass
            print("CANT PARSE", e, mapDict.loc[mapDict['custom_edit_url'] == key][field].values)
    if len(output) > 0:
        output = "<!--\n" + output + "-->\n"
        # print(output)
